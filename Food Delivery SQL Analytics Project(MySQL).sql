...
from orders 
where month(placed_at) between 2 and 12 and year(placed_at)=2025)
group by customer_code
having count(*)=1
order by customer_code;


#---4- Growth team is planning to create a trigger that will target customers after their every third order with a personalised communication and they asked you to create a query for this.
...
#---6- What percent of customers were organically acquired in Jan 2025 (placed their first order without promo code)
...
select count(
case
 when rn=1 and promo_code_name is null then customer_code 
 end)*100/count(distinct customer_code) as organic_customers_percent
from cte;